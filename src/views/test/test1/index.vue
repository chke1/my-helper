<template>
  <c-card>
    <p>重点事件属性</p>
    <table>
      <tr v-for="(item, index) in familiarAttributes" :key="index">
        <td>{{ item.name }}</td>
        <td>{{ item.desc }}</td>
      </tr>
    </table>
    ...
    <p>点</p>
    <table>
      <tr v-for="(item, index) in pluginData" :key="index">
        <td>{{ item.name }}</td>
        <td>{{ item.desc }}</td>
      </tr>
    </table>
...
const familiarAttributes = ref([
  ...
  {
    name: 'html渲染原理（dom树和css树），以及加载顺序',
    desc: '',
  },
  {
    name: 'flex-shrink: 0;',
  ...
  {
    name: '生成器 (Generator) 和 迭代器 (iterators)',
    desc: '',
  },
  {
    name: '阅读文档，阅读项目源码',
...
const familiarMethod = ref([
  {
    name: 'v-bind="{}" 传递属性',
    desc: '透传 Attributes 使用，子组件为一个根节无意触发了Vue3透传Attributes机制没有出现报错，可当你的子组件中有多个根节点时，Vue使用透传Attributes机制也没有办法确定要在哪一个根节点继承属性时，就报出警告',
  },
  {
    name: 'Prop 透传 ',
    desc: 'provide inject',
