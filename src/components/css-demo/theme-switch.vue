<template>
  <label class="theme-switch" id="themeSwitchId" :style="{ '--toggle-size': `${size}px` }">
    <input
      type="checkbox"
      :checked="modelValue"
...
<script setup>
const emit = defineEmits(['update:modelValue', 'on-change', 'on-rect']);
...
const onThemeChange = () => {
  setTimeout(() => {
    const el = document.getElementById('themeSwitchId');
    const { width, height, x, y } = el.getBoundingClientRect();
    emit('on-rect', { x: x + width / 2, y: y + height / 2 });
  }, 200);
